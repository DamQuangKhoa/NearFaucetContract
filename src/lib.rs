...
impl FaucetContract {
    #[init]
    pub fn new(owner_id: AccountId, ft_contract_id: AccountId, max_shared: U128) -> Self {
        FaucetContract {
            owner_id: owner_id,
            ...
            max_share_per_account: max_shared.0,
            is_paused: false,
        }
    }

    #[payable]
    pub fn get_token(&mut self, amount: U128) -> Promise {
        assert!(
        ...
        ext_ft_contract::ft_transfer(
            account_id.clone(),
            amount,
            Some(String::from("Faucet by LINE Dev")),
            &self.ft_contract_id,
            1,
            FT_TRANSFER_GAS,
        )
